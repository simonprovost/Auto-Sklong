...
def _load_dataset(
    ...
    longitudinal_dataset.set_data(longitudinal_dataset.data.sample(frac=0.05, random_state=random_state))
...
def _split_dataset_by_kfold(
    ...
    if os.path.exists(dataset_file_path) and \
            all([
                os.path.exists(f"{os.path.splitext(dataset_file_path)[0]}_split_{i}_train.csv") for i in
        ...
        return f"{os.path.splitext(dataset_file_path)[0]}_split_{fold_number}_train.csv", \
    ...
    for i, (train_index, test_index) in enumerate(folds.split(data), 1):
        ...
        print(f"Split {i} created - Train data saved to: {train_path}, Test data saved to: {test_path}")
...
class ExperimentEngine:
    """
    The ExperimentEngine class is responsible for running nested cross-validation experiments on a given dataset using an AutoML system.

    This class is designed to be flexible, allowing for the use of various AutoML systems or custom algorithms that adhere to the required interface.
    The results of each outer fold of the nested cross-validation are saved separately and can be manually merged afterward.
    ...
    Attributes:
        ...
        system_hyperparameters (dict[str, Any], Optional): Hyperparameters for configuring the AutoML system. By Default, will be used AutoSklong with the provided hyperparameters.
    ...
    Methods:
        ...
        _setup_system():
            Configures the AutoML system or custom algorithm to be used in the experiment.
        run_experiment():
    ...
    def __init__(
        ...
        Args:
            ...
            setup_data_parameters (dict[str, Any]): A dictionary containing necessary parameters for data setup. Must include:
            ...
            system_reporter (Callable,): A callable for reporting the results of the system.
                - The reporter function takes the system as input, as well as the test set, and return a dictionary of experiment results, where the object returned must contain:
                    - "predictions": The predicted labels for the test set.
                    - "probability_predictions": The predicted probabilities for each class.
                    - "best_pipeline": A dictionary containing the best pipeline configuration found by the system. The object must look as follows:
            ...
            system_hyperparameters (dict[str, Any]): A dictionary of hyperparameters for the AutoML system. See further in `_setup_system`.

        Raises:
            ValueError: If any required setup_data_parameters are missing or invalid. As well as, if any required system_hyperparameters are missing or invalid.
        """
        ...
        for param in required_data_parameters:
            if param not in setup_data_parameters:
                raise ValueError(f"The {param} is required in setup_data_parameters. Missing: `{param}`")
        ...
        if not system_hyperparameters.get("custom_system", False):
            ...
            for param in required_system_parameters:
                if param not in system_hyperparameters:
                    raise ValueError(f"The {param} is required in system_hyperparameters. Missing: `{param}`")
    ...
    def _setup_output_directory(self) -> None:
        ...
        Raises:
            ValueError: If the directory for the current fold already exists.
        """
        current_directory = os.getcwd()
        directory_name = self.output_path
        directory_split_name = f"{directory_name}/fold_{self.fold_number}"

        if os.path.exists(f"{current_directory}/{directory_name}"):
            print(f"Output directory {current_directory}/{directory_name} already exists. "
                  f"Will only try to create the fold directory.")
        else:
            os.makedirs(f"{current_directory}/{directory_name}", exist_ok=True)
            print(f"Output directory {current_directory}/{directory_name} created.")

        if os.path.exists(f"{current_directory}/{directory_split_name}"):
            raise ValueError(f"Output directory {current_directory}/{directory_split_name} already exists.")
        else:
            os.makedirs(f"{current_directory}/{directory_split_name}", exist_ok=True)
            print(f"Output directory {current_directory}/{directory_split_name} created.")

        self.output_path = f"{current_directory}/{directory_split_name}"
    ...
    def _setup_system(self):
        ...
        If a `custom_system` is provided in the system_hyperparameters, it is used directly. Otherwise, the Auto-Sklong is
        ...
        if self.system_hyperparameters.get("custom_system", False):
            ...
            if hyperparameters.get("output_folder", False):
                hyperparameters[hyperparameters.get("output_folder")] \
                    = f"{self.output_path}/fold_{self.fold_number}_" + time.strftime("%Y%m%d-%H%M%S")
                hyperparameters.pop("output_folder")
            elif hasattr(self.system_hyperparameters["custom_system"], "output_folder"):
                hyperparameters["output_folder"] \
                    = f"{self.output_path}/fold_{self.fold_number}_" + time.strftime("%Y%m%d-%H%M%S")
            else:
                print("No output folder provided for the custom system. If needed, please provide it through"
            ...
            print(f"Will skip the system setup.")
        else:
            ...
            self._system = GamaLongitudinalClassifier(
                ...
                random_state=self.setup_data_parameters["random_state"],
                output_directory=f"{self.output_path}/fold_{self.fold_number}_" + time.strftime("%Y%m%d-%H%M%S"),
    ...
    def report_experiment(self) -> None:
        ...
        required_results = ["predictions", "probability_predictions", "best_pipeline", "metric_optimised"]
        ...
        fieldnames = [
            "Fold", "AUPRC", "AUROC", "Macro AVG Precision", "Macro AVG Recall", "Macro AVG F1 Score",
            "Precision (Positive Class)", "Recall (Positive Class)", "F1 Score (Positive Class)",
            "Data Preparation", "Preprocessor", "Classifier", "Metric Optimised"
        ]
